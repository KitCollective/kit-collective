...
`scripts/check-mobile-tab-bar.mjs` (CI via `pnpm check:mobile-tab-bar`) guards the locked native tab bar in `apps/mobile/app/(tabs)/_layout.tsx`: it fails unless the layout renders `NativeTabs` with the five triggers in the locked order (`collection`, `inbox`, `search`, `wishlist`, `profile`), Søg is the center tab, every tab carries its Danish label, and capture stays the Samling header action (`collection-header.tsx` `Tilføj trøje` / `add`) rather than a tab. Prevents repeating the KIT-23 checker fail #4 (text-only tabs) and the 2026-09-05 native-tabs regression risks (drifted order, capture re-added as a tab). Tighten only. *(Superseded the pre-2026-09-05 floating-capsule `tabBarIcon` check when the Tab bar lock moved to native tabs.)*
...
`scripts/check-mobile-tab-bar.mjs` exposes `tabTriggerNames(source)` to count and order the `NativeTabs.Trigger` route names. `scripts/tests/check-mobile-tab-bar.test.mjs` (CI via `node --test` in `.github/workflows/ci.yml`) imports `tabTriggerNames` and `checkMobileTabBar` from the real script (mirroring `check-pr-write-scope.test.mjs` → `scripts/lib/pr-write-scope.mjs`) and mutation-tests that removing a trigger drops the count below five, breaks the locked order, and that re-adding an `add`/`capture` trigger fails `checkMobileTabBar({ layoutSource })`. Prevents repeating the KIT-42 checker round 3 fail (ratchet weakened to substring match only) and round 6 fail (hand-duplicated counting logic in the self-test), now retargeted at the native-tabs contract. Tighten only.
...
`apps/mobile/src/testing/wishlist-ui-evidence.ts` with vitest in `apps/mobile/tests/wishlist-ui-evidence.test.ts` (CI via `pnpm --filter @kit/mobile test -- wishlist-ui-evidence`) fails when `apps/mobile/src/components/wishlist-screen.tsx` exists without the paired pure-logic module `apps/mobile/src/components/wishlist-sheet-logic.ts`, vitest file `apps/mobile/tests/wishlist-sheet.test.ts` importing that module with locked AC markers (Sheet titles `Ønske` / `Ny ønskerække`, empty-state one-sentence body via `resolveWishlistEmptyBody`, Gem disabled until a criterion), when the sheet uses an empty `EmptyState` body, a nested Sheet for season pick instead of `SeasonPickerOverlay` + `ListRow`, omits `borderSubtle` hairlines between grouped manage rows, when `apps/api/src/wishlist/wishlist.service.ts` `writeToValues` mints dummy `id`/`userId` or returns `WishlistRow` instead of criteria-only values, or when `packages/domain/src/wishlist.ts` carries narrating JSDoc on `hasWishlistCriterion` / `buildWishlistAndMeta` / `buildWishlistAutoName`. The same vitest file mutation-tests the ratchet via `checkMobileWishlistUiEvidence` overrides. Prevents repeating the KIT-133 checker fail (empty-state anatomy, nested season Sheet, list-row hairlines, API slop). Tighten only.
...
### Ratchet script path registry (KIT-98)

`scripts/tests/check-ratchet-script-paths.mjs` (CI via direct `node` invocation in `.github/workflows/ci.yml`; logic covered by `scripts/tests/check-ratchet-script-paths.test.mjs`) fails when any committed `scripts/check-*.mjs` or `scripts/tests/check-*.mjs` path is missing from `RATCHET_SCRIPT_PATHS` in `scripts/lib/pr-write-scope.mjs`. Prevents repeating the KIT-118 write-scope retry loop (branch added a new ratchet but the image allowlist lagged). Tighten only.

### Factory lock ratchet (KIT-81 / KIT-98)

`scripts/tests/check-factory-locks.mjs` (CI via direct `node` invocation in `.github/workflows/ci.yml`; logic covered by `scripts/tests/check-factory-locks.test.mjs`) fails when `.cursor/rules/orchestration.mdc` treats Done as merge approval or omits Merging as merge permission; when `factory.config.example.json` omits Merging (`started`) between Ready for merge and Done; when generated `CONTEXT.md` runtime drifts to Cloud Agents dispatch; or when the required GitHub `test` job drops the factory-lock steps. Complements `scripts/tests/land-policy.test.mjs` (which locks `factory.config.json` and merge gate logic) and `scripts/tests/check-signal-up-create-contract.mjs` (Triage create contract). Prevents repeating KIT-76/KIT-61 factory-lock leftovers that could not land in the KIT-76 write-scope. Tighten only.
