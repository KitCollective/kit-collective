...
`scripts/check-mobile-paywall-iap.mjs` (CI via `node scripts/check-mobile-paywall-iap.mjs`) fails when `apps/mobile/src/premium/native-store-billing.ts` omits a SAFETY-justified `require("react-native-iap")` while `apps/mobile/src/components/paywall-sheet.tsx` exists, when the paywall dock uses the wrong Button variants (Månedlig primary, Årlig secondary, Gendan køb tertiary — not secondary for Restore), when paywall source hardcodes kroner display prices, omits `typography.mono` for store prices, or when `apps/mobile/src/premium/store-billing-client.ts` `require`s the native IAP module without a `SAFETY:` justification. `scripts/tests/check-mobile-paywall-iap.test.mjs` (CI via `node --test`) mutation-tests the ratchet. Prevents repeating the KIT-134 checker fail (dropped store SDK seam, wrong Button dock variants, hardcoded prices, and anti-slop SAFETY miss on native IAP require). Tighten only.
