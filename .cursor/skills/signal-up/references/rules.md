...
2. Create a Linear issue: status `dispatch.state`, label `signal-up` only, same project, related to the origin. `blockedBy` the origin only if it truly cannot start until this lands. Include title, description, acceptance criteria, and why it is out of scope. Do not also apply `needs-triage` while Linear’s Triage group is exclusive (a human may split that group; until then two Triage labels on one issue are rejected).
