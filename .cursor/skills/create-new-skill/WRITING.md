...
Matt’s `writing-for-agents` body. The packaging differs; the writing does not: the same levers make each document predictable — the agent taking the same *process* every run, not producing the same output.

When the document you're writing is a skill, also read [`SKILL-MECHANICS.md`](SKILL-MECHANICS.md).
