...
- Follow `docs/design-system.md` when it exists. Flag gaps; do not invent tokens, variants, or visual rules. On Expo screens, also read `.cursor/skills/expo/expo-native-ui` (and `expo-ui` when native controls fit); the design lock still wins.
- Tab bar anatomy is icon + Danish label on every tab. Empty state uses the lock’s type roles (`type.title` / `type.body`), not nearby tokens that “look close”.
