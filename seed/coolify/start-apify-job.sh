#!/usr/bin/env bash
# Start the Apify seed one-shot Coolify service via the Coolify MCP `control` tool.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
SERVICE_UUID="${1:-${SEED_COOLIFY_SERVICE_UUID:-}}"
ACTION="${2:-start}"

if [[ -z "$SERVICE_UUID" ]]; then
  echo "start-apify-job: pass service UUID or set SEED_COOLIFY_SERVICE_UUID" >&2
  exit 1
fi
...
case "$ACTION" in
  start | stop | restart) ;;
  *)
    echo "start-apify-job: action must be start, stop, or restart" >&2
    exit 1
    ;;
esac
