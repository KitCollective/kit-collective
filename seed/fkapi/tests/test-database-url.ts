...
/**
 * Recognizably disposable Postgres for seed/fkapi integration tests.
...
export function resolveSeedFkapiTestDatabaseUrl(): string {
  ...
  if (!isRecognizablyTestDatabase(url)) {
    throw new Error(
      "SEED_FKAPI_TEST_DATABASE_URL must point at a disposable test database (localhost, 127.0.0.1, or a *test* database name). Do not use the shared development DATABASE_URL for resetTestDatabase.",
    );
  }
  return url;
}
