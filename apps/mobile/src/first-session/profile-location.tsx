...
type ProfileLocationProps = {
  onBack: () => void;
  onSaved: (location: { countryLabel: string | null; city: string | null }) => void;
};

export function ProfileLocation({ onBack, onSaved }: ProfileLocationProps) {
  ...
  async function saveLocation(patch: { countryId?: string | null; city?: string | null }) {
    ...
    try {
      ...
      await refreshUser();
      onSaved({ countryLabel: updated.countryLabel, city: updated.city });
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.canvas }]}>
      <View style={{ paddingTop: insets.top }}>
        <DrillHeader title={PROFILE_LOCATION_LABEL} onBack={onBack} />
      </View>
        ...
        <ProfileSurfaceGroup>
          {countries.map((entry, index) => (
            <View key={entry.id}>
              ...
              <ListValueRow
                title={entry.label}
                meta={entry.id === countryId ? PROFILE_CURRENT_COUNTRY_META : undefined}
            ...
            <ProfileSurfaceGroup>
              ...
              {showFreeTagRow ? (
                <>
                  ...
                  <ListValueRow
                    title={profileFreeTagTitle(cityQuery.trim())}
