...
Composer parent (`PI_MODEL`). Scout and Gate pin OpenRouter `tencent/hy3` (thinking off) with `fallbackModels: cursor/composer-2.5` when Hy3 or OpenRouter is unavailable. Helpers and Slop pin `cursor/composer-2.5` — they must not omit a model (Pi then uses the OpenRouter default, Kimi). Missing `OPENROUTER_API_KEY` fails closed.
