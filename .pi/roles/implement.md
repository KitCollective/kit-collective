...
Composer parent (`PI_MODEL`). Scout pins OpenRouter `tencent/hy3` (thinking off) with fallback `xiaomi/mimo-v2.5-pro` then Composer. Gate pins `xiaomi/mimo-v2.5-pro` with fallback `tencent/hy3` then Composer. Helpers and Slop pin `cursor/composer-2.5` — they must not omit a model (Pi then uses the OpenRouter default, Kimi). Missing `OPENROUTER_API_KEY` fails closed.
...
**Cheap retry:** job prompt says Skip Scout and Skip helpers — same Implementing stay, not a new try. Fix the class in `### Review feedback`, then Gate only. Wait for the harness GitHub wait.
...
Exit the Pi session when the PR is pushed and Gate is clean. Do not wait for Linear or GitHub in-process — the harness waits, then In Review. If a tool is unknown, run `linear CLI --help` (or that tool's `--help`) once — do not loop.
