...
export function reviewFeedbackHasFindings(body) {
  ...
  return lines.some((line) => !/^-\s*\(none\)\s*$/i.test(line));
}
...
export async function completeChecker(input) {
  ...
  if (!linked) {
    throw new Error("checker requires a linked GitHub PR");
  }
  ...
  while (true) {
    ...
    if (now() >= deadline) {
      throw new Error("checker: timed out waiting for required GitHub checks");
    }
  ...
  if (piFindings) {
    feedbackLines.push(
      ...reviewFeedbackSection(workpadBody)
        .split("\n")
        .map((line) => line.trim())
        .filter(Boolean),
    );
