...
- Health: `https://harness.eskobar.dev/health` → `{"ok":true,"planner":"active","job":null,"capacity":{"ramFreeMb":…,"diskFreeMb":…,"ready":true|false}}` when the process is up. HTTP 200 always while the worker is running — never 503 because a coding job is live, hung, or waiting on the Capacity gate. `job` is `{ role, identifier }` once a sibling slice reports the coding slot; until then it is `null`.
