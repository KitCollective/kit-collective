...
- Implement worktrees: bare mirror `/var/lib/kit-pi/mirror.git`, issue trees `/var/lib/kit-pi/worktrees/KIT-n`. Checkout sits on the open `development` PR head for that identifier, else `origin/kit-n`. Implement may create `kit-n` from `origin/development` only when neither exists. Checker reuse throws instead of reviewing the lane. Two open issue PRs fail closed. One issue, one open PR. Compose volume `kit_pi`.
