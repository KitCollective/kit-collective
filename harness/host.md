...
- Implement worktrees: bare mirror `/var/lib/kit-pi/mirror.git`, issue trees `/var/lib/kit-pi/worktrees/KIT-n`. Implement creates `kit-n` from `origin/development`. Factory-checker reuses that tree (or `origin/kit-n` once the PR exists). One issue, one branch, one PR. Checker pass updates the existing workpad with an all-good status line and token use; fail writes `### Review feedback`. Compose volume `kit_pi`.
