...
- **Playwright Chromium (image vs Pi package):** The harness Dockerfile installs Playwright’s `chromium-headless-shell` binary and Debian deps at image build (`playwright install --with-deps chromium-headless-shell`). That is the headless Chromium runtime — not Desktop Chrome. The in-repo `kit-implement-browser` Pi package is **not** in global `pi install` or `.pi/settings.json`; UI implement loads it via `--skill` only. Planner, Intake, factory-checker, and api/db-only implement never get browser tools. GitHub Actions fakes the browser adapter and never opens Chromium.
- Capacity gate: before a coding-job spawn, free RAM and the `kit_pi` worktree volume must clear env floors (defaults 2048 MB RAM / 5120 MB disk). Names: `PI_CAPACITY_RAM_MB`, `PI_CAPACITY_DISK_MB`. When `ready` is false the job stays queued, status is unchanged (not Timeout park), and the worker writes one Linear comment (`## Capacity gate`) updated in place. Planner still runs. Chromium for implement UI evidence counts extra (`CHROMIUM_RAM_MB`, 512 MB on top of the RAM floor). If that Chromium floor is not ready, the job may still spawn Pi but browser tools are omitted and Chromium spawn is refused. Headless Chromium only — never Nicklas’s Desktop Chrome or a personal browser profile.
