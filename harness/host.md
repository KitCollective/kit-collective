...
- Implement worktrees: bare mirror `/var/lib/kit-pi/mirror.git`, issue trees `/var/lib/kit-pi/worktrees/KIT-n`. Checkout sits on the open `development` PR head for that identifier, else `origin/kit-n`. Implement may create `kit-n` from `origin/development` only when neither exists. A leftover `rebase-merge` is aborted and the tree is force-reset onto that PR head. implement-exit rebases only when the open PR is not already `MERGEABLE`; a conflicted rebase is aborted instead of leaving the tree wedged. Implement-exit still runs after a non-zero Pi so a green MERGEABLE PR can move In Review. Checker reuse throws instead of reviewing the lane. Two open issue PRs fail closed. One issue, one open PR. Checker pass updates the existing workpad with an all-good status line and token use; fail writes `### Review feedback`. Compose volume `kit_pi`.
...
Resume is a Linear CLI job on its own mutex (`startResumePoller`, same interval as the planner poller). It must not wait behind a hung planner or Intake. On listen it lists Implementing / In Review / Ready for merge / Merging and enqueues the matching factory role when the coding slot does not already hold that identifier. Implement enqueues when Linear Agent is empty. Checkout reuses the Issue worktree. Parked and Implementing with Cursor Agent are skipped. Write-scope overlap among Implementing issues enqueues only the first in Linear priority order. It does not claim, move status, or set delegate.
