...
**Session progress (Pi stdout):** while implement or factory-checker runs, `pi-session-log.mjs` emits structured `phase`, `tool`, and `tokens` events (Scout/Gate/helpers, bash/read, `memory_search` / `session_search` query, `memory_add|replace|remove` target only — never lesson bodies, live token snapshots every **15s**). Stop-points 1–10: session=1, scout=2, helper=3, gate=4, implement=6, checker=8. Full JSON stays in Grafana log details (click row). Grafana dashboard auto-refresh defaults to **5s**; the stat **Session events (last 1m)** is a rolling count — not the poll interval.
