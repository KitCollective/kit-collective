...
3. Jobs: `pi --model cursor/composer-2.5` (implement parent) or `pi --model cursor/grok-4.6` (factory-checker / land). Planner is the Linear CLI wrapper, not a Pi session. Defaults: `PI_MODEL` / `PI_MODEL_FAST`. Optional `HARNESS_MODEL_PROFILE=economy|balanced|premium` (default `balanced`): **economy** is OpenRouter-only — parent `--model` plus temporary pins on Scout/Draft/helpers/Slop use free/cheap OpenRouter (never `cursor/composer-*`); factory-checker/land use `openrouter/tencent/hy3`; pins restore after Pi exits. **premium** keeps Composer even on simple. **balanced** sets free parent for simple only; critical + helpers stay Composer. Implement Scout pins OpenRouter `tencent/hy3` (no-think) with fallback `xiaomi/mimo-v2.5-pro` then Composer (balanced/premium). Gate pins `xiaomi/mimo-v2.5-pro` with fallback `tencent/hy3` then Composer — not Kimi, not Hy4. Helpers and Slop pin `cursor/composer-2.5` on disk (Pi does not inherit the parent `--model` when the agent omits `model:`); economy rewrites those pins for the stay. Put `OPENROUTER_API_KEY` in this host `.env` (wizard or `/opt/kit-collective/harness/.env`).
