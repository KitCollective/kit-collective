...
- **Worker memory:** Hermes store at `/var/lib/kit-pi/hermes` on the same `kit_pi` volume — outside Issue worktrees, survives Worktree reap and image rebuild. **This slice is Memory readers only:** implement, Scout, Gate, and helpers may search; memory-write tools and `skill_manage` are excluded from implement spawn. Injection is Memory policy-only (no MEMORY.md dump). Factory-checker as Memory writer lands in KIT-112. `PI_CODING_AGENT_DIR=/workspace/.pi/agent` (committed reader config); `KIT_PI_HERMES=/var/lib/kit-pi/hermes`. Git ratchets still win over Hermes.
