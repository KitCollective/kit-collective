...
- Health: `https://harness.eskobar.dev/health` → `{"ok":true,"planner":"active","job":null,"jobs":[…],"queued":[…],"capacity":{"ramFreeMb":…,"diskFreeMb":…,"ready":true},"tokens":{"role":"implement","identifier":"KIT-n","costUsd":0.012,"lines":[{"role":"implement","model":"Composer","modelId":"cursor/composer-2.5","input":…,"output":…,"costUsd":…}]},"tokenRuns":[…]}` HTTP 200 while the process is up. **`tokens`** is the last coding-job snapshot; **`tokenRuns`** is a ring (newest first, cap 20) with per-model input/output and list-rate `costUsd` estimates. Structured harness logs also emit `event:"tokens"` per run for Loki. Durable history lands in SQLite at `KIT_TOKEN_DB_PATH` (default `/var/lib/kit-pi/token-runs.sqlite` on the `kit_pi` volume) with identifier, issueId, sessionId, model lines, and costUsd — query with `node token-report.mjs KIT-n` inside the webhook container. Cursor-routed Composer/Grok costs use list rates (or a reported usage.cost when Pi emits one). Never API keys.
...
| `red` | Do not merge / stuck | `fail` (idle timeout, Pi exit, queue throw), checker-fail `exit` |
...
**Session progress (Pi stdout):** while implement or factory-checker runs, `pi-session-log.mjs` emits structured `phase`, `tool`, and `tokens` events for Grafana (Scout/helpers, bash/read, `memory_search` / `session_search` query, `memory_add|replace|remove` target only — never lesson bodies, live token snapshots every **15s**). Bash non-zero is **yellow** (command kept via toolCallId), not loopRisk red. Gate spawn fail is yellow superseded noise. Helper fails keep the agent name when the end event omits `args.agent`. The same Pi JSON is mapped to a display-only Linear Agent Session (`thought` / `action` / `response`) when `LINEAR_PI_ACCESS_TOKEN` is set — human verb phrases, never raw CLI. Inbound session webhooks stay unused. Stop-points 1–10: session=1, scout=2, helper=3, gate=4, implement=6, checker=8. Full JSON stays in Grafana log details (click row). Grafana dashboard auto-refresh defaults to **5s**; the stat **Session events (last 1m)** is a rolling count — not the poll interval.
...
# Token / cost runs (phase=token-run — model lines in .models, costUsd on the event)
{compose_service="webhook", event="tokens"} | json | phase="token-run" | line_format "{{.identifier}} · {{.role}} · in={{.tokensIn}} out={{.tokensOut}} · {{.detail}}"
...
- Implement worktrees: bare mirror `/var/lib/kit-pi/mirror.git`, issue trees `/var/lib/kit-pi/worktrees/KIT-n`. Checkout sits on the open `development` PR head for that identifier, else `origin/kit-n`. Implement may create `kit-n` from `origin/development` only when neither exists. A leftover rebase is aborted; implement with an open PR force-resets onto that head. implement-exit rebases only when the open PR is not already `MERGEABLE`; a MERGEABLE PR is reset onto `origin/<branch>` before write-scope so the diff matches the PR. A conflicted rebase is aborted instead of leaving the tree wedged. Implement-exit still runs after a non-zero Pi so a green MERGEABLE PR can move In Review. Gate is superseded — implement does not spawn it. Mechanical close / Green light runs `pnpm format:check` / `biome ci .` (red) then applies `pnpm format` / `biome check --write`, and on outdated lockfile runs `pnpm install --lockfile-only` + commit (`classifyCiFailure` → `format` | `lockfile` | `migration` | `logic` | `unknown`). Clean-tree is required before rebase; CONFLICTING is rebased during the GitHub wait (cap 3, then full-Builder feedback). Workpad honesty claims are checked against git/CI before In Review. After helpers, implement spawns Optimizer (`.pi/agents/optimizer.md`); checker Slop stays `.pi/agents/slop.md`. Typed cheap-retry (`shouldSlimCheapRetry`) uses slim Pi only for mechanical classes — logic/unknown get full Builder. Out-of-glob write-scope keeps Implementing and **re-runs implement in the same slot** (cheap retry, not a try) so the issue is not dropped behind a resume overlap skip. When an in-scope `package.json` is in the PR diff, `pnpm-lock.yaml` is a write-scope companion (not a violation). implement-exit evaluates write-scope with the image allowlist as the floor; the worktree `scripts/lib/pr-write-scope.mjs` may waive only new `scripts/check-*.mjs` / matching tests. A colliding Drizzle `NNNN_` prefix vs `origin/development` cheap-retries (`migrationRetry`) before rebase wait. `.cursor/hooks/block-pi-ci-sleep.sh` denies `sleep` ≥ 10s and `gh pr checks` poll/`--watch` — the harness waits. CI, write-scope, and format retries **Skip Scout** and skip helpers: prompt is workpad `### Review feedback` plus the redacted CI excerpt (fix the class — format vs Zod vs unique-email). **Logic/unknown CI red or PR still missing after one exit-only try** → **Optimizer resume** (Skip Scout/Draft/helpers, spawn optimizer once) then harness Mechanical close / GitHub wait again — same Implementing stay until green. **First PR-create fail** → workpad + in-slot **exit-only** `mechanicalExitFastPath` (0 Pi). **Back from Review (checker fail)** → **Builder resume** (Skip Scout; helpers only if findings need a seam; then Optimizer; exit; wait until green). Cheap retries are not tries: they do not increment `reviewLoops` / `ciFailCycles`, do not post a retry-cap hold, and do not make resume skip. `createFormatCheck` treats `maxBuffer` / ENOBUFS as infra, not format-red. After the cheap-retry bound the slot yields (stay Implementing) so resume can continue. Checker reuse throws instead of reviewing the lane. Two open issue PRs fail closed. One issue, one open PR. Checker pass updates the existing workpad with an all-good status line and token use; fail writes `### Review feedback`. Same Review-feedback fingerprint ≥ `STUCK_FEEDBACK_CAP` → Parked + `ready-for-human`. Spec-green requires Evidence/Validation/AC ticks. **Factory-checker token save:** harness injects a capped issue description + three-dot diff into `checker-append.md` before Pi spawn; readonly `git` bash is allowlisted only for gaps; `gh pr checks` stays harness-owned (not Pi). Compose volume `kit_pi`.
...
Resume is a Linear CLI job on its own mutex (`startResumePoller`, same interval as the planner poller). It must not wait behind a hung planner or Intake. On listen it lists Implementing / In Review / Ready for merge / Merging and enqueues the matching factory role when the coding slot does not already hold that identifier. Implement enqueues when Linear Agent is empty and the issue is labelled `ready-for-agent`. Checkout reuses the Issue worktree. Parked, missing `ready-for-agent`, `ready-for-human` / `needs-info` / `wontfix`, Implementing with Cursor Agent, and In Review with workpad `workpad-incomplete-parked` (incomplete Spec/Standards/Slop after in-slot checker retries) are skipped. Stale **implement retry cap** comments do not skip enqueue (KIT-125). When an implement job completes without a Linear status change, the identifier drops out of `queued`; the next resume tick may enqueue the same issue again (including merge-fail while still Implementing). Write-scope overlap among Implementing issues enqueues only the first in Linear priority order; overlap-skip is intentional — a blocked issue waits for the overlapping implement to finish or for a human to adjust scope. It does not claim, move status, or set delegate.

Incomplete checker workpad (missing Spec/Standards/Slop axes) while the PR is green stays **In Review** and re-runs factory-checker in-slot (cap 2); it does not bounce to a full implement tree. After the cap, the workpad is parked for human and resume/webhook skip re-enqueue.
...
3. Jobs: `pi --model cursor/composer-2.5` (implement parent) or `pi --model cursor/grok-4.6` (factory-checker / land). Planner is the Linear CLI wrapper, not a Pi session. Defaults: `PI_MODEL` / `PI_MODEL_FAST`. Optional `HARNESS_MODEL_PROFILE=economy|balanced|premium` (default `balanced`): **economy** is OpenRouter-only (never `cursor/composer-*`): Scout/helpers/Slop → Hy3; Builder simple → DeepSeek Flash else Hy3; Optimizer → DeepSeek Flash; ui-ux → GLM 5.3 Flash (multimodal); Draft free → Flash → Hy3; factory-checker/land → Hy3; pins restore after Pi exits. **premium** keeps Composer even on simple. **balanced** sets free parent for simple only; critical + helpers stay Composer. **Profile switches models, not honesty / Composition / scaffold / Green light gates** — those stay fail-close on every profile. Implement Scout pins OpenRouter `tencent/hy3` (no-think) with fallback `xiaomi/mimo-v2.5-pro` then Composer (balanced/premium). Gate pins `xiaomi/mimo-v2.5-pro` with fallback `tencent/hy3` then Composer — not Kimi, not Hy4. Helpers, Optimizer, and Slop pin `cursor/composer-2.5` on disk (Pi does not inherit the parent `--model` when the agent omits `model:`); economy rewrites those pins for the stay. Put `OPENROUTER_API_KEY` in this host `.env` (wizard or `/opt/kit-collective/harness/.env`).
