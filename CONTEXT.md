...
**Draft**:
One Pi scaffold subagent after workpad `### Composition` and before Composer domain helpers. Pins free OpenRouter coding models (`minimax/minimax-m3:free`, then `z-ai/glm-5.2:free`, then `poolside/laguna-s-2.1:free`) with Hy3 then Composer fallbacks. Writes boilerplate under write-scope only (stubs, types, rote shells, Composition mirrors). Parent and nest/drizzle/expo/ui-ux/devops still own correctness, TDD green, and sensitive seams. Skip Draft on cheap/Spec/first-pass resume (with Skip Scout), when Model route says Skip Draft (critical), and when the slice is auth/IAP/Vision-only.
